...
for sess_name in sess_names:
    ...
    sorting_dirs = get_sorting_dirs(ephys_dir, date_str, dir1_name, dir2_name)
    ...
    split_recordings,split_sortings = [[obj.frame_slice(start, start+n_frames)
                                       for start, n_frames in zip(np.cumsum(np.pad(segment_info['n_frames'], [1, 0])),
                                                                  segment_info['n_frames'])] for obj in (concat_rec[0],
                                                                                                         concat_sorting[0])]
    # assert False
    [sorting.save(folder=sort_dir.parent/f'from_concat',overwrite=args.ow_flag)
     for sorting, sort_dir in zip(split_sortings, sorting_dirs) if not (sort_dir.parent/f'from_concat').is_dir() or args.ow_flag]
