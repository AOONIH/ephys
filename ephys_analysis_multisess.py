...
from neural_similarity_funcs import plot_similarity_mat, get_list_pips_by_property, get_sim_mat_over_time,get_reordered_idx
...
if __name__ == "__main__":
    ...
    # if name == 'all':
    #     name = session_topology.name.unique()
    # else:
    #     name = [name]
    # if date == 'all':
    #     date = session_topology.date.unique()
    # else:
    #     date = [date]
    date = int(date)
    all_sess_info = session_topology.query('name==@name & date==@date')
    ...
    ephys_figdir = ceph_dir/'Dammy'/'figures'/'rerun_w_std_fix'
    ...
    for (_,sess_info),spike_dir in zip(all_sess_info.iterrows(), sort_dirs):
        ...
        if not sessions[sessname].sound_event_dict:
            ...
            [np.save(sess_psth_dir / f'{key}_psth.npy', get_predictor_from_psth(sessions[sessname], key, psth_window,
                                                                                ...
                                                                                use_iti_zscore=True))
             for key in sessions[sessname].sound_event_dict]
        ...
        #     sessions[sessname].get_licks_to_event(normal[0],'A-0')
        ...
        if sess_info['sess_order'] == 'main':
            ...
            if 5 in sessions[sessname].td_df['Stage'].values:
                # get event idx
                ...
                for permute, plots in zip([False],pip_by_pip_sim_plot[1]):
                    for pi, pip2use in enumerate('ABCD'):
                        ...
                        reordered_names, reordered_idxs = get_reordered_idx(pip_desc,pip_lbls,['name'],subset=[p for p in pip_lbls if pip2use in p])
                ...
                pip_by_pip_sim_plot[0].set_size_inches(32,8)
                pip_by_pip_sim_plot[0].show()
                pip_by_pip_sim_plot[0].savefig(ephys_figdir / f'by_pip_similarity_{sessname}{"_permute" if permute else ""}_reordered.svg')
